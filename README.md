...
Current and latest release is on the `:dev` branch. Latest stable releases are at `v3pX` branches - according to the available pre-releases of VRS.
...
Requests for additions to the LocalAircraft.txt go [in the rokegale/LocalAircraft repo](https://github.com/rikgale/LocalAircraft/issues).
...
| `VRS_CULTURE` | see [microsoft's page](http://msdn.microsoft.com/en-us/goglobal/bb896001.aspx) for a list of supported culture names. Not all translations may be available | `unset` means `en-GB` |
...
| `VRS_ENHANCED_LAYERS_CONFIG` | The configuration for the default map, enabled layers and menu button position. See the docker-compose.yml example above! | `unset` defaults to example values |
| `VRS_DB_UPDATE_BACKUP_UNCOMPRESSED` | Works only in conjunction with `VRS_DB_UPDATE_POLICY_FULLAUTO`. Prior the update a backup of the database is created. If this is set, the backup will remain uncompressed | `unset` |
| `VRS_ENHANCED_MARKERS` | Installs and activates VRS custom markers. `normal` is for the ADS-B set and `disable` deactivates the markers. [rikgale/VRSCustomMarkers](https://github.com/rikgale/VRSCustomMarkers) has more details about the markers. | `unset` |
| `VRS_ENHANCED_LAYERS_COUNTRY` | Installs and activates VRS enhanced layers. Takes a country code as input. Currently available are `UK`, `DE`, `USA1`, `USAAZ`, `SE`,`AU`,`IN`,`ID`,`NL`,`FR`,`ES`. [rikgale/VRSCustomLayers](https://github.com/rikgale/VRSCustomLayers) contains more details, feel free to open an issue there to get your country on board. | `unset` - Unknown country codes will default to UK |
| `VRS_ENHANCED_LAYERS_OPENWX_APIKEY` | For the enhanced weather layers to work, you need an API key. Again,  [rikgale/VRSCustomLayers](https://github.com/rikgale/VRSCustomLayers) has more details, and instructions how to obtain an API key. | `unset` |
| `VRS_ENHANCED_LAYERS_OPENAIP_APIKEY` | For the OpenAIP layers to work, you need an API key. Again,  [rikgale/VRSCustomLayers](https://github.com/rikgale/VRSCustomLayers) has more details, and instructions how to obtain an API key. | `unset` |
| `VRS_ENHANCED_LAYERS_STADIA_APIKEY` | For the [Stadia map](https://docs.stadiamaps.com) to work, you need an API key. Again,  [rikgale/VRSCustomLayers](https://github.com/rikgale/VRSCustomLayers) has more details, and instructions how to obtain an API key. | `unset` |
| `VRS_ENHANCED_LAYERS_THUNDERFOREST_APIKEY` | For the [Thunderforest map](https://www.thunderforest.com) to work, you need an API key. Again,  [rikgale/VRSCustomLayers](https://github.com/rikgale/VRSCustomLayers) has more details, and instructions how to obtain an API key. | `unset` |
...
* [Additional Aircraft Markers](https://github.com/rikgale/VRSCustomMarkers) (rikgale)
